...
from bitcash.transaction import (
    calc_txid, create_p2pkh_transaction, sanitize_tx_data, sanitize_slp_tx_data,
...
class PrivateKey(BaseKey):
    ...
    def create_slp_token(self, ticker, token_name, document_url,
                        document_hash, decimals, baton_vout, token_quantity, 
                        fee=None, leftover=None, combine=True,
                        unspents=None, custom_pushdata=False
                        ):  # pragma: no cover


        op_return = slp_create.buildGenesisOpReturn(
            ticker, token_name, document_url,
            document_hash, decimals, baton_vout, token_quantity
        )
        ...
        outputs = (self.address, 546, "satoshi")
    ...
    def send_slp(self, outputs, tokenId, fee=None, leftover=None, combine=False,
        ...
        tx_hex = self.create_slp_transaction(
            outputs, tokenId, fee=fee, leftover=leftover, combine=combine, message=message, unspents=unspents
        )
...
class PrivateKeyRegtest(BaseKey):
    ...
    def create_slp_token(self, ticker, token_name, 
                        document_url, document_hash,
                        decimals, token_quantity, 
                        token_address=None, mint_baton_address=None,
                        fee=None, leftover=None, combine=True,
                        unspents=None, custom_pushdata=False
                        ):  # pragma: no cover
